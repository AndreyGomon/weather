...
import ru.mascot.features.weather.data.utils.DateDay
...
data class WeatherData(
    ...
    fun getPoint(): GeoPoint = GeoPoint(latitude, longitude)
    ...
    fun toProtobuf(): ProtoWeatherData = ProtoWeatherData(
        code = code.code.toInt(),
        latitude = latitude,
        longitude = longitude,
        locality = locality,
        provider = provider,
...
data class HourlyList(
    ...
    fun next24Hours(): Last24Hours {
        ...
        val filtered = list.filter { data ->
            val ts = data.timestamp ?: return@filter false
...
data class WeatherStat(
    ...
    fun getExpired(current: Long = System.currentTimeMillis()): Boolean {
        return current - requestsStart >= 86_400_000L
    }

    fun resetExpired(current: Long = System.currentTimeMillis()): WeatherStat {
